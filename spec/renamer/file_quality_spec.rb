...
describe FileQuality do
  ...
  context "when sources are equivalent" do
    ...
    it "ranks one file quality over another if resolution, version and quality all improve or stay the same" do
      expect(FileQuality.new(common_info)).to be < FileQuality.new(common_info.merge(:video_resolution => '1920x1080', :version => 3))
      expect(FileQuality.new(common_info)).to be > FileQuality.new(common_info.merge(:video_resolution => '640x480', :quality => 'med'))
      expect(FileQuality.new(common_info)).to be > FileQuality.new(common_info.merge(:version => 1, :quality => 'med'))
      expect(FileQuality.new(common_info)).to be < FileQuality.new(common_info.merge(:video_resolution => '1920x1080', :version => 3, :quality => 'very high'))
    end

    it 'ranks as equal file qualities where resolution, version and quality move in different directions' do
      expect(FileQuality.new(common_info)).to be_equivalent_to FileQuality.new(common_info.merge(:video_resolution => '1920x1080', :version => 1))
    end  

    it "remains true for sources that are not equal but of the same score" do
      expect(FileQuality.new(common_info)).to be > FileQuality.new(common_info.merge(:quality => 'med', :source => 'HKDVD'))
    end  
  end
  ...
  context "when sources are not equivalent" do
    ...
    it "ranks one file quality as better than another if resolution and/or quality move the same way" do
      expect(FileQuality.new(common_info)).to be > FileQuality.new(common_info.merge(:source => 'VHS', :video_resolution => '640x480'))
      expect(FileQuality.new(common_info)).to be < FileQuality.new(common_info.merge(:source => 'Blu-ray', :quality => 'very high'))
    end

    it "ranks as equal file qualities where resolution and/or quality move in the opposite direction of source" do
      expect(FileQuality.new(common_info)).to be_equivalent_to FileQuality.new(common_info.merge(:source => 'VHS', :video_resolution => '1920x1080'))    
      expect(FileQuality.new(common_info)).to be_equivalent_to FileQuality.new(common_info.merge(:source => 'Blu-ray', :quality => 'low'))    
      expect(FileQuality.new(common_info)).to be_equivalent_to FileQuality.new(common_info.merge(:source => 'Blu-ray', :quality => 'low', :video_resolution => '1920x1080'))    
    end  

    it "ignores version" do
      expect(FileQuality.new(common_info)).to be > FileQuality.new(common_info.merge(:source => 'VHS', :version => 3))
      expect(FileQuality.new(common_info)).to be < FileQuality.new(common_info.merge(:source => 'Blu-ray', :version => 1))
    end  
  end  
end  