...
describe MylistData do
  it 'provides a list of the normal episodes' do
    data = MylistData.new :title=>"Binchou-tan", :unknown_ep_list=>"", :episodes=>"12", :hdd_ep_list=>"1-5,7,9,11", :deleted_ep_list=>"", :cd_ep_list=>"", :single_episode => false
    data.normal_episodes.should match_array([1,2,3,4,5,7,9,11])
  end
  
  it 'generates a union of the unknown, hdd and cd eps' do
    data = MylistData.new :title=>"Binchou-tan", :unknown_ep_list=>"9", :episodes=>"12", :hdd_ep_list=>"1-5,7", :deleted_ep_list=>"", :cd_ep_list=>"2", :single_episode => false
    data.normal_episodes.should match_array([1,2,3,4,5,7,9])
  end  
  
  it 'generates a union that also contains special episodes' do
    data = MylistData.new :title=>"Binchou-tan", :unknown_ep_list=>"", :episodes=>"12", :hdd_ep_list=>"1-5,7,9,11", :deleted_ep_list=>"", :cd_ep_list=>"C1-C3,T01", :single_episode => false
    data.normal_episodes.should match_array([1,2,3,4,5,7,9,11])
  ...
  it 'is always complete if there is a single episode' do
    data = MylistData.new :lid=>51718530, :fid=>"43501", :eid=>"16368", :aid=>"1516", :gid=>"183", :date=>"1219466009", :state=>"1", :viewdate=>"0", :storage=>"", :source=>"", :other=>"", :filestate=>"0", :single_episode => true
    data.should be_complete
    data.add(10)
